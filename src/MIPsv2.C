...
//const int Nside=1;
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void TrackSeek(Int_t nHits,Int_t *Layer,Int_t *Bar,Int_t *ADC,Double_t PedestalPar[][2016],Int_t *Dy,Int_t *Side,Int_t *GID,Int_t FitTrack[15])
{
  Float_t MaxADC[14];
  ...
  for(int i=0; i<nHits;i++)//only check side 0
	...
	{
	  if (ADC[i]>(PedestalPar[0][GID[i]]+3*PedestalPar[1][GID[i]]))
	    {
	      ADCCutPed=ADC[i]-PedestalPar[0][GID[i]];
...
*/
  ...
  for (int ng=0;ng<nGID;ng++)
    {
      ...
      EMC_Mips[ng]=new TH1F(cc,cc,300,10,3000-iside*1500);
    }
  ...
  Double_t MIPsPar[nLangaus][nGbar];
  for(int ig=0;ig<nLangaus;ig++)
    {
  ...
  Long64_t timestop=0;
  //  Long64_t M_time[2]={0,0};
  Double_t PedestalPar[2][2016];

  char zFileName[80];
  memset(zFileName,0,sizeof(zFileName));
  Long64_t ztimess[2]={0,0};
  

  Long64_t ientry = LoadTree(0);
  nb = fChain->GetEntry(0); 
  bool ReGetPed=true;
  GetPedPar(PedestalPar,zFileName,ztimess,time);
  cout<<"We are getting a Pedestal rootfile: "<<zFileName<<endl;
  cout<<"the starttime is: "<<ztimess[0]<<endl;
  cout<<"the stoptime is: "<<ztimess[1]<<endl;
  if(ztimess[1]<time)
  ReGetPed=false;
	...
	for(int i=0;i<nHits;i++)
	  {
		...
		{
		  if(mode==0||mode==1)
		EMC_Mips[GID[i]]->Fill(ADC[i]-PedestalPar[0][GID[i]]);
		}
	  }
      else 
	{BadTrack++;
	  ...
	  for(int ib=0;ib<Nbar;ib++)
	    {
	      ...
	      EMC_Mips[iGID]->Fit(myMIPs,"R");
              if(iGID%3==2)
              for(int ipar=0;ipar<nLangaus;ipar++)
	      MIPsPar[ipar][(int)(iGID/3)]=myMIPs->GetParameter(ipar);
	      // cout<<"@@@@@@@@@@@@@"<<"   "<<ztimess[0]<<"    "<<ztimess[1]<<"  "<<PedestalPar[0][iGID]<<"  "<<PedestalPar[1][iGID]<<endl;
  ...
  Long64_t M_time[2]={timestart,timestop};
  if(mode==0||mode==1) 
  SetMipPar(MIPsPar,RootFileName,M_time);
