...
int main(int argc,char *argv[])
{
  ...
  string outputfile="./Raw2ROOT"+filename+namesuffix;
  ...
  Int_t event=0;
  Int_t trigger=0;
  Int_t mode=0;//0:normal 1:compress 2:calibration
  Int_t nHits=0;
  Int_t FEE_ID[5000];
  Int_t Chan[5000];
  Int_t ADC[5000];
  Int_t Layer[5000];
  Int_t Bar[5000];//1-22
  Int_t Side[5000];// 1:front or left 2:rear or right
  Int_t Dy[5000];
  Int_t GID[5000];//GID =((layer*2+side)*24+bar)*3;  
  EMCTree->Branch("time",&time,"time/L");
  EMCTree->Branch("mtime",&mtime,"mtime/S");
  EMCTree->Branch("event",&event,"event/I");
  EMCTree->Branch("mode",&mode,"mode/I");
  EMCTree->Branch("trigger",&trigger,"trigger/I");
