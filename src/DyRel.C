...
EMC_DyRel[ng]=new TH2F(cc,cc,500,0,700,3000,0,16500);
}
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  calped.runtime=45555555;
  calpedIO.GetPedPar(&calped);
...
{
    if(Lmax[Layer[i]]<=(Bar[i]+1)&&Lmax[Layer[i]]>=(Bar[i]-1)&&Bar[i]!=0&&Bar[i]!=23){//both sides
    if (ADC[i]>(calped.PedMean[GID[i]]+2.5*calped.PedSigma[GID[i]]))
      dynode[GID[i]]=ADC[i]-calped.PedMean[GID[i]];
}
}
...
{ 
if(dynode[i]!=0&&dynode[i-1]!=0)
  EMC_DyRel[i]->Fill(dynode[i-1],dynode[i]);
if(dynode[i]!=0&&dynode[i-2]!=0)
