  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
    {
      ...
      if (ientry < 0) break;
      //if(time<41960500 || time>41963300) continue;
      //cout<<time<<endl;
	...
	for(int idy=0;idy<Ndy;idy++)
	  {
		...
		//
		//Fill
		if(ibar==0||ibar==Nbar-1)
		  for(int npeak=0;npeak<13;npeak++)
		    FEE_G[iGID]->Fill(Cal_Vol[npeak],pp[npeak]);

		else
		  for(int npeak=0;npeak<12;npeak++){
		    //EMC_Cal[iGID]->Rebin(4);
		    mygaus->SetRange(pp[npeak]-150,pp[npeak]+150);
		    EMC_Cal[iGID]->Fit(mygaus,"R0Q");
		    mygaus->GetParameters(GausPar);

		    mygaus->SetRange(GausPar[1]-3*GausPar[2],GausPar[1]+3*GausPar[2]);
		    EMC_Cal[iGID]->Fit(mygaus,"R0Q");
		    mygaus->GetParameters(GausPar);
		    FEE_G[iGID]->Fill(Cal_Vol[npeak],GausPar[1]);
		    memset(GausPar,0,sizeof(GausPar));
		  }
