...
//  PedCalSig[ng]->Fill(Ast2Dat((calped.timestop+calped.timestart)/2),calped.PedSigma[ng]);
  ...
  for (int ng=0;ng<nGID;ng++){
   ...
   // cout<<SigmaMax[ng]<<"  "<<SigmaMin[ng]<<endl;
    ...
    ShowMean[is][id]->SetBinContent(ib+1,il+1,(int)((MeanMax[ng]-MeanMin[ng])*100)/100.+0.01);
    ShowSigma[is][id]->SetBinContent(ib+1,il+1,(int)((SigmaMax[ng]-SigmaMin[ng])*100)/100.+0.01);
